...
fn ordenar_vector(vector: &[u8], result: &mut [u8]) {
    let periodos: BTreeSet<&[u8]> = vector
        .chunks(TAM_PERIODO)
        .filter(|p| p != &PERIODO_NULO)
        .collect();
...
fn main() -> Result<(), std::io::Error> {
    ...
    let entrada = BufReader::with_capacity(64 * 1024, file);
    let file = File::create(&args[2])?;
    let mut salida = BufWriter::with_capacity(64 * 1024, file);
