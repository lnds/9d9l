...
object Main {
  ...
  def compress(inputFile: String, outputFile: String) : Unit = {
    ...
    val huffTree = HuffTree.build(calcFrequencies(bytes))
  ...
  private[this] def calcFrequencies(bytes: Array[Byte]) : List[(Char, Int)] = {
    val freqs = Array.fill[Int](HuffTree.maxSymbols)(0)
    bytes.foreach(symbol => freqs(symbol & 0xFF) += 1)
    freqs.zipWithIndex.map{case (f, s) => (s.toChar, f)}.filter(_._2>0).toList
  }
